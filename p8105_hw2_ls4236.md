...
First of all, load the dataset `pols-month` and clean it, and tidy the
data according to the first step.
...
Second of all, load the dataset `snp` and clean it, and tidy the data
...
Third of all, load the dataset `unemployment` and clean it, and tidy the
data according to the third step.
...
`unemployment` dataset contains 816 observations with monthly
...
Overall, the combined Trash Wheel dataset contains 1188 observations of
trash collection events from Baltimore’s three water-wheels, with
Mr. Trash Wheel contributing the majority of records (707) since 2014,
followed by Gwynnda (349) since 2021 and Professor Trash Wheel (132)
since 2017.

Key variables include:

- Trash weight, with a mean of 3.0619276 tons per dumpster, ranges from
  0.61 to 5.62tons per dumpster
- Trash volume, with a mean of 15.047138 cubic yards per dumpster and
  statistical mode 15, ranges from 5 to 20 cubic yards per dumpster
- Specific trash items like cigarette butts with an average of
  1.2275314^{4} per month, and plastic bottles, which is up to 9830
  items per dumpster
- Homes Powered, with a mean of 47.2070707 per dumpster.

In addition, Professor Trash Wheel records a total of 282.26 tons of
...
```
    ...
    ## Warning in left_join(zillow_df, zip_df, by = "zip_code"): Detected an unexpected many-to-many relationship between `x` and `y`.
...
```
    ...
    ## # A tibble: 2 × 5
    ##   county_zip county_code county_fips zip_code neighborhood             
    ##   <chr>      <chr>       <chr>       <chr>    <chr>                    
    ## 1 Bronx      005         36005       10463    Kingsbridge and Riverdale
    ## 2 New York   061         36061       10463    Kingsbridge and Riverdale
...
```
    ...
    ##   county_zip county_code county_fips zip_code neighborhood      
    ##   <chr>      <chr>       <chr>       <chr>    <chr>             
    ## 1 Kings      047         36047       11201    Northwest Brooklyn
    ## 2 New York   061         36061       11201    Northwest Brooklyn

This is probably due to the fact that 11201 is mostly in Kings County
(Brooklyn), but part of it overlaps into New York County (Manhattan).
...
In addition, there are some mismatches between `county_zillow` and
...
```
    ...
    ## # A tibble: 1 × 12
    ##    year month   day region_id_zillow size_rank zip_code county_zillow  zori
    ##   <int> <int> <int> <chr>                <int> <chr>    <chr>         <dbl>
    ## 1  2024     8    31 62181                 8012 11693    Queens         2800
    ## # ℹ 4 more variables: county_zip <chr>, county_code <chr>, county_fips <chr>,
    ## #   neighborhood <chr>

Merge again, relocate, and arrange to get new `combined_zillow` whose
number of rows (10450) equals to that of `zillow` datasets (10450).
...
The resulting tidy dataset contains 10450 observations and 12 variables.
Specifically,
...
Here are the those ZIP codes only appear in `zip_df` dataset but not in
`zillow_df` dataset: 10464, 10474, 10475, 10499, 10550, 10704, 10705,
...
| zip_code | borough | neighborhood | zori_2020 | zori_2021 | price_change | percent_change |
|:---|:---|:---|---:|---:|---:|---:|
| 10007 | Manhattan | Lower Manhattan | 6334.211 | 5421.614 | -912.5966 | -14.40742 |
| 10069 | Manhattan | NA | 4623.042 | 3874.918 | -748.1245 | -16.18251 |
| 10009 | Manhattan | Lower East Side | 3406.442 | 2692.187 | -714.2550 | -20.96778 |
| 10016 | Manhattan | Gramercy Park and Murray Hill | 3731.135 | 3019.431 | -711.7045 | -19.07474 |
| 10001 | Manhattan | Chelsea and Clinton | 4108.098 | 3397.648 | -710.4499 | -17.29389 |
| 10002 | Manhattan | Lower East Side | 3645.416 | 2935.113 | -710.3028 | -19.48482 |
| 10004 | Manhattan | Lower Manhattan | 3149.658 | 2443.697 | -705.9608 | -22.41389 |
| 10038 | Manhattan | Lower Manhattan | 3573.201 | 2875.616 | -697.5853 | -19.52270 |
| 10012 | Manhattan | Greenwich Village and Soho | 3628.566 | 2942.344 | -686.2218 | -18.91165 |
| 10010 | Manhattan | Gramercy Park and Murray Hill | 3697.284 | 3012.353 | -684.9304 | -18.52523 |
