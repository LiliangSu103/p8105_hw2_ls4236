...
First of all, load the dataset `pols-month` and clean it, and tidy the data according to the first step.
...
Second of all, load the dataset `snp` and clean it, and tidy the data according to the second step.
...
snp_df =
  ...
  mutate(
    year = as.integer(year),
    year = ifelse(year < 16, 2000 + year, 1900 + year), # here convert year into 4 digit value
    year = as.integer(year),
    month = as.integer(month),
    day = as.integer(day)
  ) |> 
...
Third of all, load the dataset `unemployment` and clean it, and tidy the data according to the third step.
...
The `pols-month` dataset includes `r nrow(pol_df)` observations of `r ncol(pol_df)` dimentions(variables) tracking the number of national politicians monthly by party affiliation, spanning from 1947 to 2015. The key variables include presidential party, number of governors, senators, and representatives of both parties. The `snp` dataset provide `r nrow(snp_df)` observations of monthly S&P 500 closing values from 1950 to 2015, which can be seen as an indicator of stock market performance. The `unemployment` dataset contains `r nrow(une_df)` observations with monthly unemployment rate spanning from 1948 to 2015.

After cleaning and merging, the resulting dataset combines these data into a comprehensive dataset with `r nrow(merge_df)` rows and `r ncol(merge_df)` variables, from 1947 to 2015. Key integrated variables include presidential party affiliation (converted to a single president variable), S&P 500 closing values (close), unemployment rates (une_rate), and detailed counts of politicians by party.
...
```{r}
mr_wheel = read_excel(
  "./data/202509_Trash_Wheel_Collection_Data.xlsx", 
...
- Trash weight, with a mean of `r combined_wheel |> pull(weight_tons) |> mean(na.rm = TRUE)` tons per dumpster, ranges from `r combined_wheel |> pull(weight_tons) |> min(na.rm = TRUE)` to `r combined_wheel |> pull(weight_tons) |> max(na.rm = TRUE)`tons per dumpster
- Trash volume, with a mean of `r combined_wheel |> pull(volume_cubic_yards) |> mean(na.rm = TRUE)` cubic yards per dumpster and statistical mode 15, ranges from `r combined_wheel |>  pull(volume_cubic_yards) |> min(na.rm = TRUE)` to `r combined_wheel |> pull(volume_cubic_yards) |> max(na.rm = TRUE)` cubic yards per dumpster
- Specific trash items like cigarette butts with an average of `r combined_wheel |> pull(cigarette_butts) |> mean(na.rm = TRUE)` per month, and plastic bottles, which is up to `r combined_wheel |> pull(plastic_bottles) |> max(na.rm = TRUE)` items per dumpster
- Homes Powered, with a mean of `r combined_wheel |> pull(homes_powered) |> mean(na.rm = TRUE)` per dumpster.
...
```{r}
combined_zillow = left_join(zillow_df, zip_df, by = "zip_code")
```
...
zip_df |> count(zip_code) |> filter(n>1) |> pull(zip_code)
zip_df |> filter(zip_code == "10463")
zip_df |> filter( zip_code == "11201")

```

This is probably due to the fact that 11201 is mostly in Kings County (Brooklyn), but part of it overlaps into New York County (Manhattan). After double checking, I decide to remove the rows where 11201 corresponds to Manhattan and 10463 corresponds to Manhattan.
...
In addition, there are some mismatches between `county_zillow` and `county_zip`. After double checking, I decide to change the corresponding `county_zip` into same values as `county_zillow`.

```{r}
combined_zillow[which(pull(combined_zillow, county_zillow) != pull(combined_zillow, county_zip)), ]
```

Merge again, relocate, and arrange to get new `combined_zillow` whose number of rows (`r nrow(combined_zillow)`) equals to that of `zillow` datasets (`r nrow(zillow_df)`).
...
combined_zillow = left_join(zillow_df, zip_df, by = "zip_code") |> 
  ...
  relocate(
    ...
    region_id_zillow, size_rank, county_zip, county_code, county_fips # Others
  ) |> 
...
```{r}
missing_zips = anti_join(zip_df, zillow_df, by = "zip_code")
```

Here are the those ZIP codes only appear in `zip_df` dataset but not in `zillow_df` dataset: `r missing_zips |> pull(zip_code)`.
